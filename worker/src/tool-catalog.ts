...
export const GROUPED_TOOL_DEFS: GroupedToolDef[] = [
  ...
  {
    "name": "news_events",
    "description": "Company news, official IR RSS/Atom, registry-reviewed IR pages, press releases, and public event timeline.\n\nRead coverage before absence: coverage.state is FULL/PARTIAL, with failedSources, skippedSources, and recommendedNextAction. Yahoo primary items require tickerMatch=EXPLICIT with matchBasis TICKER_TOKEN, ISSUER_NAME, or ISSUER_ACRONYM. Per-source rawCount/acceptedCount/rejectedCount distinguish fetched listings from retained evidence. For decisionUse=CHECK_OFFICIAL_RELEASES, escalate to official releases or event verification; legacy confidence is not a cross-provider quality ranking.\n\nActions:\n- get_company_news: Lightweight multi-source news. Defaults exclude company_ir and company_ir_page; pass sources to include official RSS or registry-backed IR-page diagnostics. Params: ticker, max_results, lookback_days, sources\n- search_company_news: Keyword search with compact coverage plus sourceStatus audit detail. Params: ticker, query, start_date, end_date, sources, max_results\n- get_company_press_releases: Official release events; defaults resolve SEC EX-99 first, then company_ir_page, then Yahoo PR context. Decision-grade only when payload decisionGrade=true with SEC_EX99_RESOLVED or APPROVED_IR_PAGE_RESOLVED evidence. Params: ticker, lookback_days, max_results\n- get_yahoo_finance_news: Legacy Yahoo news. Params: ticker\n- get_sec_recent_events: SEC filings as events. Params: ticker, lookback_days\n- get_public_event_timeline: Chronological timeline across SEC/company_ir/company_ir_page/newswire/Yahoo/Finnhub. Params: ticker, lookback_days\n- verify_company_event: Cross-validate event with best evidence and compact coverage. SOURCE_LIMITED_NOT_FOUND means provider coverage was incomplete, not that the event is absent. Params: ticker, event_query, start_date, end_date",
    "actions": {
