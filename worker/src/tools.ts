...
const CANONICAL_ADDITIONS: Tool[] = [
  ...
  { name: "verify_company_event", description: "Verify whether a public company event is source-backed across SEC, company_ir RSS/Atom, company_ir_page registry entries, newswire, Yahoo, and Finnhub sources. Returns CONFIRMED, PARTIAL, NOT_FOUND, STALE, or CONFLICTING with best evidence, decisionUse, and sourceStatus/sourceCoverage.", inputSchema: { type: "object", properties: { ticker: { type: "string" }, event_query: { type: "string", description: "Keywords describing the event to verify." }, start_date: { type: "string", default: "" }, end_date: { type: "string", default: "" }, sources: { type: "array", items: { type: "string" }, default: ["sec", "company_ir", "newswire", "yahoo_finance_news", "yahoo_finance_press_releases", "finnhub"] } }, required: ["ticker", "event_query"] } },
