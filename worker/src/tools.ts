...
const CANONICAL_ADDITIONS: Tool[] = [
  ...
  { name: "get_earnings_call_transcript", description: "High-level tool to retrieve earnings call transcript content from SEC 8-K exhibits. Falls back with instructions if not available via SEC.", inputSchema: { type: "object", properties: { ticker: { type: "string", description: "Stock ticker symbol" }, period: { type: "string", enum: ["latest"], default: "latest", description: "Period selector. Only 'latest' is supported." }, topics: { type: "array", items: { type: "string" }, description: "Optional list of keywords/topics to filter paragraphs by" } }, required: ["ticker"] } },
