...
const CANONICAL_ADDITIONS: Tool[] = [
  ...
  { name: "get_company_news", description: "Get recent public company news and press releases from selected public sources with source labels, timestamps, URL, dedupe metadata, decisionUse, sourceStatus, and short evidence text. Defaults stay lightweight (Yahoo Finance news/press releases + Finnhub); include source company_ir when official company website RSS/Atom discovery is needed. Accepts a single ticker or an array of up to 5 symbols; array results are returned as a per-ticker keyed object fetched independently.", inputSchema: { type: "object", properties: { ticker: { oneOf: [{ type: "string" }, { type: "array", items: { type: "string" }, maxItems: 5 }], description: "Ticker symbol (e.g. 'AAPL') or an array of up to 5 symbols (e.g. ['AAPL', 'MSFT']). If more than 5 are provided, only the first 5 are processed; split larger lists into multiple calls." }, max_results: { type: "number", default: 10 }, lookback_days: { type: "number", default: 14 }, sources: { type: "array", items: { type: "string" }, default: ["yahoo_finance_news", "yahoo_finance_press_releases", "finnhub"], description: "Allowed sources include yahoo_finance_news, yahoo_finance_press_releases, finnhub, sec, newswire, and company_ir. company_ir attempts safe official website RSS/Atom autodiscovery." } }, required: ["ticker"] } },
  { name: "search_company_news", description: "Search public company news/events for a ticker and query across selected source metadata and short snippets. Include company_ir to search official company website RSS/Atom items when discoverable; inspect sourceStatus/sourceCoverage before relying on absence.", inputSchema: { type: "object", properties: { ticker: { type: "string", description: "Ticker symbol, e.g. 'AAPL'" }, query: { type: "string", description: "Required search query string." }, start_date: { type: "string", default: "" }, end_date: { type: "string", default: "" }, sources: { type: "array", items: { type: "string" }, default: ["yahoo_finance_news", "yahoo_finance_press_releases", "finnhub"] }, max_results: { type: "number", default: 10 } }, required: ["ticker", "query"] } },
  { name: "get_company_press_releases", description: "Get company press releases and official release-style events from SEC, official company IR RSS/Atom when discoverable, Yahoo press releases, and newswire sources. Gate use is payload-level: only payloads with decisionGrade:true and resolved SEC EX-99 press-release evidence are decision-grade; RSS-only or unresolved exhibit states remain verification/context evidence.", inputSchema: { type: "object", properties: { ticker: { type: "string" }, lookback_days: { type: "number", default: 90 }, max_results: { type: "number", default: 20 }, sources: { type: "array", items: { type: "string" }, default: ["yahoo_finance_press_releases", "company_ir", "newswire", "sec"] } }, required: ["ticker"] } },
  ...
  { name: "get_public_event_timeline", description: "Get a deduplicated chronological timeline of public company events across SEC, official company IR RSS/Atom when discoverable, newswire, Yahoo, and Finnhub sources. Returns sourceStatus/sourceCoverage so missing official feeds are distinguishable from no events.", inputSchema: { type: "object", properties: { ticker: { type: "string" }, start_date: { type: "string", default: "" }, end_date: { type: "string", default: "" }, sources: { type: "array", items: { type: "string" }, default: ["sec", "company_ir", "newswire", "yahoo_finance_news", "yahoo_finance_press_releases", "finnhub"] }, max_results: { type: "number", default: 50 }, newest_first: { type: "boolean", default: false } }, required: ["ticker"] } },
  { name: "verify_company_event", description: "Verify whether a public company event is source-backed across SEC, official company IR RSS/Atom when discoverable, newswire, Yahoo, and Finnhub sources. Returns CONFIRMED, PARTIAL, NOT_FOUND, STALE, or CONFLICTING with best evidence, decisionUse, and sourceStatus/sourceCoverage.", inputSchema: { type: "object", properties: { ticker: { type: "string" }, event_query: { type: "string", description: "Keywords describing the event to verify." }, start_date: { type: "string", default: "" }, end_date: { type: "string", default: "" }, sources: { type: "array", items: { type: "string" }, default: ["sec", "company_ir", "newswire", "yahoo_finance_news", "yahoo_finance_press_releases", "finnhub"] } }, required: ["ticker", "event_query"] } },
